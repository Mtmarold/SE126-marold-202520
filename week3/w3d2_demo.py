...
with open ("text_files/class_grades.csv") as csvfile:
    ...
    for rec in file:
        ...
        test1.append(int(rec[2]))   #cast as INT for easier math later
...
for index in range(0, len(firstName)): #len() --> length of collection returns # of items
    #index --> key of the list, allows access to one specific value at a time
...
for i in range(0, len(avg)):
    total_avg += avg[i] #adds each avg value to the total avg variable
